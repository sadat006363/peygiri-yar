...
const categoryIcons = {
  ...
  cost: <CurrencyDollarIcon className="w-4 h-4" />,
...
const priorityIcons = {
  high: <ExclamationCircleIcon className="w-3 h-3" />,
  medium: <ClockIcon className="w-3 h-3" />,
  low: <CheckCircleIcon className="w-3 h-3" />,
};
        ...
        <Card className="transition-all hover:border-indigo-200">
          <div className="flex flex-col gap-2">
            <div className="flex flex-wrap justify-between items-start gap-2">
              <span className={`text-xs px-2.5 py-1 rounded-full font-medium flex items-center gap-1 ${categoryColors[item.category]}`}>
                {categoryIcons[item.category as keyof typeof categoryIcons]}
                {categoryLabels[item.category] || item.category}
              </span>
              <span className={`text-xs px-2.5 py-1 rounded-full font-medium flex items-center gap-1 ${priorityColors[item.priority]}`}>
            ...
            {item.nextAction && (
              <div className="text-xs text-green-600 font-medium flex items-center gap-1">
                <ArrowPathIcon className="w-4 h-4" /> Next Action: {item.nextAction}
              </div>
            )}
            {item.waitingFor && (
              <div className="text-xs text-orange-600 font-medium flex items-center gap-1">
                <ClockIcon className="w-4 h-4" /> Waiting for: {item.waitingFor}
              </div>
            )}
